import type { ExtractedStyles, StyleGuide } from '../types';

/**
 * Generates a Markdown style guide from extracted styles
 */
export function generateStyleGuide(styles: ExtractedStyles): string {
  const lines: string[] = [];
  ...
  if (sortedColors.length > 0) {
    const colorNames = ['Primary', 'Secondary', 'Accent', 'Background', 'Text', 'Muted', 'Border', 'Success', 'Warning', 'Error'];
  ...
  // Font families
  const sortedFontFamilies = [...styles.fontFamilies.entries()]
    .sort((a, b) => b[1] - a[1]);
  ...
  // Font sizes
  const sortedFontSizes = [...styles.fontSizes.entries()]
  ...
  // Font weights
  const sortedFontWeights = [...styles.fontWeights.entries()]
