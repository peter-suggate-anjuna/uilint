...
export class JSDOMAdapter {
   ...
   */
  private async callOllama(styleSummary: string, styleGuide: string | null): Promise<UILintIssue[]> {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn('[UILint] Failed to connect to Ollama. Is it running on localhost:11434?');
        return [];
      }
  ...
  outputWarnings(issues: UILintIssue[]): void {
    ...
    issues.forEach((issue) => {
      ...
      if (issue.currentValue && issue.expectedValue) {
        parts.push(`Current: ${issue.currentValue}, Expected: ${issue.expectedValue}`);
      }
 ...
 */
export async function runUILintInTest(root?: Element | Document): Promise<UILintIssue[]> {
